...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        A=np.matmul(X**2,np.ones((X.shape[1],num_train)))
        B=np.matmul(self.X_train**2,np.ones((self.X_train.shape[1],num_test)))
        B=np.transpose(B)
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            indices = np.argsort(dists[i,:])[:k] #indices of k nearest neighbors
